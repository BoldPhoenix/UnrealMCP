...
**✅ BATCH 1 VERIFIED LIVE on UE 5.8 — 2026-06-22, against the Ship level (125 actors):** all 8 first-batch tools passed — `log_search`, `log_categories`, `set_log_verbosity`, `get_viewport_camera`, `set_viewport_camera`, `get_selected_actors`, `select_actors`, `focus_viewport`. `set_level_viewport_camera_info` confirmed real on 5.8 (both `# VERIFY` flags cleared; camera read-back + eyes_mirror proved the viewport actually moved). (PR #1.)

**✅ BATCH 2 VERIFIED + PRUNED on UE 5.8 — 2026-06-22 (live-fired all 48 drafted tools; kept 20, dropped 28):**
- **KEPT (20) → PR #2:** editor_extra — `is_pie_running`, `start_pie`, `stop_pie`, `get_content_browser_path`, `set_content_browser_path`, `get_selected_assets`, `select_assets`, `screen_to_world`, `search_cvars`\* ; config (read) — `list_containers`, `list_categories`, `list_sections`, `get_section_property_values` (read real `default_gravity_z` = -980), `get_section_schema`\* ; `compile_live_coding` ; automation — `list_tests`, `run_tests`, `get_test_status`, `stop_tests`.  *(\* = works-but-degraded, honestly noted: `search_cvars` can't enumerate / reads 0 for unset; `get_section_schema` returns an empty property map.)*
- **DROPPED (28):** ALL of `gameplay.py` (24) — GameplayTags / GameplayCue / DataRegistry / GameFeatures subsystems are **not Python-exposed** (`AttributeError` on `unreal.GameplayTagsManager` / `DataRegistrySubsystem` / `GameFeaturesSubsystem` / `AttributeSet`); the `gas_*` actor + `bt_*` tools are *wired* (clean ASC/asset errors) but PrimalErrors has no GAS/BT content to verify against. `gameplay.py` is **shelved as a draft** (on disk, not imported/committed) — re-add + verify when the game adopts those systems. Plus `world_pos_to_screen` (NoneType during PIE — unguarded/off-screen) and the 3 config **writes** (`set_section_properties` / `save_section` / `reset_section_to_defaults` — untested + they mutate the project `.ini`); those 4 remain as unregistered, `# VERIFY`-flagged drafts in their modules.
- Method held: every dud failed **soft** (`{error, hint}`) — the bridge never wobbled. PIE trip (`start_pie` → `is_pie_running:true` → `stop_pie`) and `screen_to_world` (deprojected to a ray at the player spawn) all confirmed live.
